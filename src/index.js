...
var handlers = {
    ...
    'GetFact': function () {
        ...
        var randomMessage = randomPhrase(GET_FACT_MSG_EN);

        // Create speech output
        var speechOutput = randomMessage + randomFact;
        this.emit(':askWithCard', speechOutput, 'What do you want to do?', this.t("SKILL_NAME"), randomFact)
    },
    'GetNewYearFactIntent': function () {
        var factArr = this.t('FACTS');
        var year = this.event.request.intent.slots.FACT_YEAR.value;
        // Append valid facts to the array
        function checkYear(fact) {
            return fact.includes(year);
        }
        var yearArr = factArr.filter(checkYear);       
        ...
        if (yearArr.length < 1) {
            ...
            var randomMessage = randomPhrase(GET_FACT_MSG_EN);
            var factIndex = Math.floor(Math.random() * yearArr.length);
            var randomFact = yearArr[factIndex];
            var speechOutput = randomMessage + randomFact;
